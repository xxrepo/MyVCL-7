...
begin // +
  if Skinned then begin
    if DefaultManager.SkinCommonInfo.SmallCheckBox[cbChecked] >= 0
      then Ndx := DefaultManager.SkinCommonInfo.SmallCheckBox[cbChecked]
      else Ndx := DefaultManager.SkinCommonInfo.CheckBox[cbChecked];
    if Ndx > -1 then
...
begin
  if Skinned then begin
    ...
    case ACheckState of
      cbsUnchecked:
        if DefaultManager.SkinCommonInfo.SmallCheckBox[cbUnChecked] >= 0 then
          i := DefaultManager.SkinCommonInfo.SmallCheckBox[cbUnChecked]
        else
          i := DefaultManager.SkinCommonInfo.CheckBox[cbUnChecked];

      cbsChecked:
        if DefaultManager.SkinCommonInfo.SmallCheckBox[cbChecked] >= 0 then
          i := DefaultManager.SkinCommonInfo.SmallCheckBox[cbChecked]
        else
          i := DefaultManager.SkinCommonInfo.CheckBox[cbChecked];

      cbsGrayed:
        if DefaultManager.SkinCommonInfo.SmallCheckBox[cbGrayed] >= 0 then
          i := DefaultManager.SkinCommonInfo.SmallCheckBox[cbGrayed]
        else
          i := DefaultManager.SkinCommonInfo.CheckBox[cbGrayed];
    end;
...
begin
  if Skinned then begin
    case AButtonKind of
      cxbkComboBtn: begin
        i := DefaultManager.SkinCommonInfo.ComboBtn.SkinIndex;
        sSect := s_ComboBtn;
      end
      else begin
    ...
    case AButtonKind of
      ...
      cxbkSpinUpBtn: glIndex := DefaultManager.SkinCommonInfo.ScrollBtns[asTop].GlyphIndex;
      cxbkSpinDownBtn: glIndex := DefaultManager.SkinCommonInfo.ScrollBtns[asBottom].GlyphIndex;
      cxbkSpinLeftBtn: glIndex := DefaultManager.SkinCommonInfo.ScrollBtns[asLeft].GlyphIndex;
      cxbkSpinRightBtn: glIndex := DefaultManager.SkinCommonInfo.ScrollBtns[asRight].GlyphIndex;
      cxbkComboBtn: glIndex := DefaultManager.SkinCommonInfo.ComboBtn.GlyphIndex;
    end;
...
begin // +
  if Skinned then begin
    i := DefaultManager.SkinCommonInfo.RadioButton[AChecked];
    if DefaultManager.IsValidImgIndex(i) then begin
      Size := MkSize(DefaultManager.ma[i]);
      TmpBmp := CreateBmp32(Size);
...
begin // +
  if Skinned then begin
    i := DefaultManager.GetMaskIndex(DefaultManager.SkinCommonInfo.IndexGLobalInfo, s_RadioButtonChecked);
...
begin
  if Skinned then begin
    i := DefaultManager.GetMaskIndex(DefaultManager.SkinCommonInfo.IndexGLobalInfo, s_GlobalInfo, s_SmallIconClose);
    if i < 0 then
      i := DefaultManager.GetMaskIndex(DefaultManager.SkinCommonInfo.IndexGLobalInfo, s_GlobalInfo, s_BorderIconClose);
...
var
  ...
  begin
    with DefaultManager do
      if IsValidImgIndex(SkinCommonInfo.ScrollBtns[Side].GlyphIndex) then
        PaintGlyph(SkinCommonInfo.ScrollBtns[Side].GlyphIndex)
      else begin
        C := gd[SkinCommonInfo.ScrollBtns[Side].SkinIndex].Props[mini(State, ac_MaxPropsIndex)].FontColor.Color;
  ...
  begin
    ...
    CI.FillColor := DefaultManager.GetGlobalColor;
    CI.Bmp := nil;
    with DefaultManager.SkinCommonInfo do
...
begin
  ...
  with DefaultManager.SkinCommonInfo do begin
    if AHorizontal then
      case APart of
        sbpThumbnail: begin
...
begin
  if Skinned then begin
    i := DefaultManager.SkinCommonInfo.GripRightBottom;
    if DefaultManager.IsValidImgIndex(i) then begin
...
var
  ...
  begin
    with DefaultManager do
      if DefaultManager.IsValidImgIndex(SkinCommonInfo.ScrollBtns[Side].GlyphIndex) then begin
        p.x := (WidthOf(R) - WidthOfImage(ma[SkinCommonInfo.ScrollBtns[Side].GlyphIndex])) div 2;
        p.y := (HeightOf(R) - HeightOfImage(ma[SkinCommonInfo.ScrollBtns[Side].GlyphIndex])) div 2;
        if (p.x < 0) or (p.y < 0) then
          Exit;

        DrawSkinGlyph(TmpBmp, p, State, 1, ma[SkinCommonInfo.ScrollBtns[Side].GlyphIndex], MakeCacheInfo(TmpBmp));
      end
      else begin
        C := GetGlobalFontColor;// gd[SkinCommonInfo.ScrollBtns[Side].SkinIndex].Props[mini(State, ac_MaxPropsIndex)].FontColor.Color;
        gR := Rect(0, 0, TmpBmp.Width, TmpBmp.Height);
        if (State > 1) and ButtonsOptions.ShiftContentOnClick then
          OffsetRect(gR, 1, 1);
...
begin
  if Skinned then begin
    i := DefaultManager.GetMaskIndex(DefaultManager.SkinCommonInfo.IndexGLobalInfo, s_GlobalInfo, s_SmallIconClose);
    if i < 0 then i := DefaultManager.GetMaskIndex(DefaultManager.SkinCommonInfo.IndexGLobalInfo, s_GlobalInfo, s_BorderIconClose);
    if DefaultManager.IsValidImgIndex(i) then begin
      Result.x := WidthOfImage(DefaultManager.ma[i]);
      Result.y := HeightOfImage(DefaultManager.ma[i]);
    end
