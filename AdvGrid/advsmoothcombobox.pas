...
begin
  ...
  begin
    if not SkipKeyScrolling then
    begin
      i := FSelectedItemIndex;
      case Key of
        VK_DOWN,VK_RIGHT: i := FSelectedItemIndex + 1;//Inc(FSelectedItemIndex);
        VK_UP,VK_LEFT: i := FSelectedItemIndex - 1; //Dec(FSelectedItemIndex);
        VK_HOME: i := 0; //FSelectedItemIndex := 0;
        VK_END: i := Items.Count - 1; //FSelectedItemIndex := Items.Count - 1;
        VK_NEXT: i := FSelectedItemIndex + 5; //FSelectedItemIndex := FSelectedItemIndex + 5;
        VK_PRIOR: i := FSelectedItemIndex - 5; //FSelectedItemIndex := FSelectedItemIndex - 5;
      end;

      //FSelectedItemIndex := Min(Items.Count - 1, Max(0, FSelectedItemIndex));
      SetInternalSelectedItemIndex(Min(Items.Count - 1, Max(0, i)));
      ...
      if (Items[FSelectedItemIndex].Splitter and (FSelectedItemIndex > 0)) then
      begin
        i := FSelectedItemIndex;
        while Items[i].Splitter and (i > 0) do
        begin
          case Key of
            VK_DOWN,VK_RIGHT: Inc(i);
            VK_UP,VK_LEFT: Dec(i);
            VK_HOME: i := 0;
            VK_END: i := Items.Count - 1;
            VK_NEXT: i := i + 5;
            VK_PRIOR: i := i - 5;
          end;
        end;
