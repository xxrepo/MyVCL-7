...
type
  ...
  protected
    FPrevOfficeTabCollectionItem : TOfficeTabCollectionItem;
...
function TOfficeTabCollectionItem.GetCaption: string;
begin
  try
    if (FChildForm <> nil) and (FChildForm.Caption <> FCaption) then
      Caption := FChildForm.Caption;
  except
  end;
...
begin
  ...
  if (FAdvOfficeTabs <> nil) and (ActiveTabIndex > -1) then
    xxx := GetAdvOfficeTabs(ActiveTabIndex)
  else
    xxx := nil;
  ...
  Result.FPrevOfficeTabCollectionItem := xxx;
...
var
  ...
  ActItem: TOfficeTabCollectionItem;
begin
  if TabIndex <> OldActiveTabIndex then
    FPrevActivePageIndex := ActiveTabIndex;
...
var XXX : TOfficeTabCollectionItem;
  i, j :integer;
begin //1013
  if not (csDestroying in ComponentState) and (Sender is TForm) and (TForm(Sender).FormStyle = fsMDIChild) then
  begin
  //  if (FPrevActivePageIndex >= 0) and (FPrevActivePageIndex < FAdvOfficeTabs.Count - 1) then
 //     SetActiveTabIndex(FPrevActivePageIndex);
     XXX := GetAdvOfficeTabs(ActiveTabIndex);
     if XXX <> nil then
     begin
       if (XXX.FPrevOfficeTabCollectionItem <> nil) then
       begin
         for i := 0 to AdvOfficeTabCount - 1 do
         begin
           if GetAdvOfficeTabs(i) = XXX.FPrevOfficeTabCollectionItem then
           begin
              SetActiveTabIndex(i);
           end;
         end;
