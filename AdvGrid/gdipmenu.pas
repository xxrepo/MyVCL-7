...
begin
  ...
  begin
    ...
    begin
      ...
      if not chk and PtInGPRect(r, Point(X, Y)) and Sections[I].Items[K].Enabled then
      begin
...
begin
  ...
  begin
    ...
    begin
      r := Sections[I].Items[K].FFillRect;
      if PtInGPRect(MakeRect(r.X, r.Y + (r.Height - 17) / 2, 17, 17), Point(X, Y)) and Sections[I].Items[K].Enabled
...
begin
  ...
  for K := 0 to Items.Count - 1 do
  begin
    if (Items[K].ItemType = itBreak) or (Items[K].ItemType = itLineBreak) and (K < Items.Count - 1) then
      Inc(FCnt);
  end;
