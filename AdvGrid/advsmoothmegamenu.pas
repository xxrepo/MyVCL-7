...
begin
  ...
  case Direction of
    ...
    begin
      ...
      if cnt < MenuItems.Count then
        its := (its - ((MenuItems.Count - 1 - cnt) * ItemAppearance.MenuItemSpacing)) / (MenuItems.Count - cnt);
...
begin
  ...
  begin
    if not Assigned(MenuItems[item].frm) and MenuItems[item].Enabled and (MenuItems[item].Menu.Sections.Count > 0) then
    begin
  ...
  begin
    ...
    begin
      if PtInRect(Bounds(Round(FItemRect.X), Round(FItemRect.Y), Round(FItemRect.Width), Round(FItemRect.Height)), Point(X, Y))
        and Enabled and not Separator then
      begin
        Result := i;
        break;
      end;
