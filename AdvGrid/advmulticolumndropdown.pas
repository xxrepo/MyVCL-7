...
{$ENDIF}
  ...
  protected
    ...
    property AdvDropDown: TAdvMultiColumnDropDown read FAdvDropDown write
      SetAdvDropDown;
...
begin
  if Assigned(FAdvColGrid) then
  begin
    if not FAdvColGrid.FMouseWheelSelection then
      ItemIndex := ARow - 1; // FAdvColGrid.Row - 1;
  end;
end;
...
begin
  ...
  outputdebugstring(pchar(inttostr(fixedrows)+'->'+inttostr(acol)+':'+inttostr(arow)+':'+cs));
  ...
  begin
    ...
    outputdebugstring(pchar('fixed text:'+cs));
