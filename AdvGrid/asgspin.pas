...
function GetTimeValue_ex(AText : String): tdatetime;
var
  Pos: Integer;
begin
  Pos := 1;
  if not ScanTime(AText, Pos, Result) or (Pos <= Length(AText)) then
    Result := 0;
end;

function GetDateValue_ex(AText : String): tdatetime;
var
  Pos: Integer;
begin
 if (AText = '0') or (AText = '') then Result := Now
 else
 begin
  Pos := 1;
  if not ScanDate(AText, Pos, Result) or (Pos <= Length(AText)) then
    Result := 0;
 end;
end;
...
var
  Pos: Integer;
begin
//  Result := foMyFunc.GetTimeValue(Text);
{ try
