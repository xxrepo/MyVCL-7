...
{$IFDEF VER140} Variants, {$ENDIF}
  ...
  Mask, DBLookupEh, DBCtrlsEh, ImgList;
...
type
  TForm1 = class(TForm)
    ...
    procedure DBGridEh7SortMarkingChanged(Sender: TObject);
...
begin
  DBGridEh7.RowCount := 1000;
end;

procedure TForm1.DBGridEh7SortMarkingChanged(Sender: TObject);
var i :Integer;
    s:String;
   function DeleteStr(str:String; sunstr:String): String;
   var i:Integer;
...
begin
  s := '';
  for i := 0 to DBGridEh7.SortMarkedColumns.Count-1 do
   if DBGridEh7.SortMarkedColumns[i].Title.SortMarker = smUpEh then
     s := s + DBGridEh7.SortMarkedColumns[i].FieldName + ' DESC , '
   else
     s := s + DBGridEh7.SortMarkedColumns[i].FieldName + ', ';
  if s <> '' then s := ' ORDER BY ' + Copy(s,1,Length(s)-2);
  s := DeleteStr(s,'1');
  DataModule1.Query1.SQL.Strings[DataModule1.Query1.SQL.Count-2] := s;
  DataModule1.Query1.Close;
...
procedure TForm1.DBGridEh7TitleBtnClick(Sender: TObject; ACol: Integer;
  Column: TColumnEh);
begin
  case Column.Title.SortMarker of
    smNoneEh: Column.Title.SortMarker := smDownEh;
  ...
  DBGridEh7SortMarkingChanged(Column.Grid);
