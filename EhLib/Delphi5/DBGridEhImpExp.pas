  ...
  begin
    ...
    begin
      if GetChar(FLastChar) and (FLastChar = #10) then
        ...
        raise Exception.Create('TDBGridEhImportAsText.CheckState: Invalid text format');
    end else
      Result := itssChar;
  end else
  begin
      ...
      if Field.DataSet.State in [dsEdit,dsInsert] then
        try
          ...
          begin
            ...
            begin
              ModalResult := MessageDlg('Error during insert value: '+ #10+
                E.Message+ #10+ #10+ 'Ignore error?', mtError, [mbYes,mbNo,mbAll],0);
              case ModalResult of
                mrNo: Abort;
...
begin
  ...
  if Count < SizeOf(TVCLDBIF_BOF) then
    raise Exception.Create('TDBGridEhImportAsVCLDBIF.ReadPrefix: Invalid VCLDBIF format');
        ...
        if Field.DataSet.State in [dsEdit,dsInsert] then
          try
            ...
            begin
              if not FIgnoreAll then
              begin
                ModalResult := MessageDlg('Error during insert value: '+ #10+ E.Message+ #10+ #10+
                          'Ignore error?', mtError, [mbYes,mbNo,mbAll],0);
                case ModalResult of
                  mrNo: Abort;
                  mrAll: FIgnoreAll := True;
                end;
...
begin
  ...
  if Count < SizeOf(Byte) then
    raise Exception.Create('TDBGridEhImportAsVCLDBIF.ReadDataCell: Invalid VCLDBIF format');
