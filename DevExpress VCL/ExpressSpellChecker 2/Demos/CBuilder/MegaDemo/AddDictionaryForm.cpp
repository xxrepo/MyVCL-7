...
{
  TdxSpellCheckerLanguages *ALanguages;
  TdxSpellCheckerCodePages *ACodePages;
  int I;
  ALanguages = new TdxSpellCheckerLanguages();
  try
  {
	for (I = 0; I < ALanguages->Count; I++) {
	  cbLang->Properties->Items->AddObject(ALanguages->Name[I], (TObject*)ALanguages->LocaleID[I]);
	}
	cbLang->ItemIndex = ALanguages->IndexOf((unsigned)ALanguages->GetDefaultLanguageLCID());
  }
  __finally
  {
	ALanguages->Free();
