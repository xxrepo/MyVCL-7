...
//---------------------------------------------------------------------------
__fastcall TfrmDataSets::TfrmDataSets(TComponent* Owner): TfrmDemoBaisicMain(Owner)
{
//
}
//---------------------------------------------------------------------------
...
void TfrmDataSets::UpdateSummaryChartView()
{
  ...
  try
  {
	...
	for (I = 0; I < PivotGridSummaryDataSet->FieldCount; I++)
	{
	  ...
	  {
		case faColumn, faRow: SummaryChartView->CreateDataGroup()->DataBinding->FieldName = PivotGridSummaryDataSet->Fields->Fields[I]->FieldName; break;
