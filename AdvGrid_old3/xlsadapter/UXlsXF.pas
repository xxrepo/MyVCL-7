...
const
  ...
  ('', '0', '0.00','#,##0','#,##0.00',                              //0..4
   ...
   'd/m/YYYY','DD-MMM-YY','DD-MMM','MMM-YY',                      //14..17
...
begin
  Result:=TFlxDiagonalBorder((GetCardinal(Data,12) shr 14) and  3);
end;
...
begin
  ...
  Result.Font.CharSet:=Font.CharSet;
  ...
  Result.FillPattern.FgColorIndex:=CellFgColorIndex+1;
  Result.FillPattern.BgColorIndex:=CellBgColorIndex+1;
...
begin
  for i:=0 to Count-1 do
    ...
    for i:= Low(XlsBuiltInFormats) to High(XlsBuiltInFormats) do
  ...
  if Count=0 then  NewId:=High(XlsBuiltInFormats)+1 else NewId:= Items[Count-1].Id+1;
  Result:=Items[Add(TFormatRecord.CreateFromData(Fmt, NewId))].Id;
end;
