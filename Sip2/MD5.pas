...
begin
  Result := LowerCase(MD5DigestToStr(MD5String(v)));
end;

function AutResponse (Const Method, Phone, Server, User, Realm,
  Password, Qop, nonce, Opaque, EntityBody: AnsiString): AnsiString;
var
  A1, A2, Uri: AnsiString;
begin
  if Length(Phone) = 0 then Uri := 'sip:'+Server
    else Uri := 'sip:'+Phone+'@'+Server;

  A1 := md(User+':'+Realm+':'+Password);
  If Qop='auth-int' then A2:=md(Method+':'+Uri+':'+md(EntityBody))
    Else A2:=md(Method+':'+Uri);
  if Length(Qop) = 0 then
    Result := md(A1+':'+nonce+':'+A2)
  else
    Result := md(A1+':'+nonce+':00000001:'+Opaque+':'+Qop+':'+A2);
  Result:=Result+''; // absolutely needless
end;
